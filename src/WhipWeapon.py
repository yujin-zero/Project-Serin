    ...
    def attack(self):
        ...
        if now - self.last_shot > self.shoot_delay[self.level]:
            ...
            if self.serin.direction < 0:
                ...
                images = [pygame.transform.flip(image, True, False) for image in self.images]
                whip = Whip(self.serin.rect.centerx -100, self.serin.rect.centery-30, base_angle, self.damage[self.level], images)
...
class Whip(pygame.sprite.Sprite):
    def __init__(self, x, y, angle, damage, images):
        super().__init__()
        self.images = [pygame.transform.scale(image, (image.get_width() // 2, image.get_height() // 2)) for image in images]
        self.image_index = 0
    ...
    def update(self):
        self.image = pygame.transform.rotate(self.images[self.image_index], -math.degrees(self.angle))
